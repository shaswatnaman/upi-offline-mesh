 ...
 * devices that are nearby (all devices are within range for the demo) and
 * copies packets between them, decrementing TTL each hop.
 *
 * When a device with internet (a "bridge node") holds a packet, the demo's
 * /api/mesh/flush endpoint causes it to POST that packet to the backend —
 * simulating the moment a phone walks outside and gets 4G.
 */
@Service
public class MeshSimulatorService {
     ...
     * is equivalent to fast-forwarding N rounds of pairwise gossip.
     */
        ...
        for (VirtualDevice src : deviceList) {
            for (MeshPacket pkt : snapshot.get(src.getDeviceId())) {
                ...
                for (VirtualDevice dst : deviceList) {
                    ...
                    dst.hold(pkt.withDecrementedTtl());
                    transfers++;
                }
            }
        }
